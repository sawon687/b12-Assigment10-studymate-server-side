...
async function run() {
  try {
    ...
    app.get('/userProfile', async (req, res) => {
      const {experienceSort,limit,search}= req.query; 
      console.log('experience',limit,search)
         const query={}
         if(search)
         {
            query.$or=[
      ...
      const result = await userProfileColl.find(query).limit(Number(limit)|| 0).toArray();
      const Totalcount=await userProfileColl.c
