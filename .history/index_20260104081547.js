...
async function run() {
  try {
    ...
    app.get('/userProfile', async (req, res) => {
      const {experienceSort,limit,search}= req.query.experienceSort; // Beginner / Intermediate / Expert
         const query={}
         if(search)
         {
            query.search=search;
         }
      const result = await userProfileColl.find(query).limit(limit.toArray();
